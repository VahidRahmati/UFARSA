function [output_UFASAR opt_out] = reoncstruction_UFASAR(fluors,opt)
...
if ZeroEvent_flag ~=1 % when there are non-zero amount of detected events
    ...
    output_UFASAR = build_trains(EventTimes_SR,vector_nSpikes_discrete_SR,idx_onset_Actu_B_SR,ZeroEvent_flag);
    
    
else
    output_UFASAR = build_trains([],[],[],ZeroEvent_flag);
end


% write the main parameters of UFASAR into its output variables
output_UFASAR.leading_thr = opt.thr_leading;
output_UFASAR.std_noise = opt.std_noise;
output_UFASAR.min_leading_thr_est = opt.min_thr_leading_est;
output_UFASAR.interior_thr = thr_wburst;
opt_out = opt;
...
%% 
    function  output_UFASAR = build_trains(EventTimes_SR,vector_nSpikes_discrete_SR,idx_onset_Actu_B_SR,ZeroEvent_flag)
        % Reconstruction of spiking activity trains: build the final output-vectors of the reconstruction process
        ...
        output_UFASAR.eTrain = EventTrain_rec(1:nFrames);
        output_UFASAR.eTrain_dem = EventTrain_rec_SuperResol(1:nFrames);
        output_UFASAR.cTrain = CountTrain_discrete_rec(1:nFrames);
        output_UFASAR.cTrain_dem = CountTrain_discrete_rec_SuperResol(1:nFrames);        
        
        if opt.demerging == 0 % when no Demerging was applied
            output_UFASAR.eTrain_dem = [];
            output_UFASAR.cTrain_dem = [];
        end
            
        % fluorescence trace in different versions
        output_UFASAR.fluors.beforeSmoothing = fluors.beforeSmoothing(1:nFrames);
        output_UFASAR.fluors.original = fluors.original(1:nFrames);
        output_UFASAR.fluors.afterSmoothing = fluors.afterSmoothing(1:nFrames);
        output_UFASAR.fluors.normalized = fluors.normalized(1:nFrames);
        
        if opt.gen_FR_count == 1
            output_UFASAR.cFR = gen_smoothed_FR(opt.sigma_gauss, output_UFASAR.cTrain); 
        else
            output_UFASAR.cFR = []; 
        end
        
        if opt.gen_FR_count_dem == 1 && opt.demerging == 1
            output_UFASAR.cFR_dem = gen_smoothed_FR(opt.sigma_gauss, output_UFASAR.cTrain_dem);
        else
            output_UFASAR.cFR_dem = [];
        end
...
vector_nSpikes_discrete = frac_amp_floor + [(frac_amp-frac_amp_floor)>=0.75];
