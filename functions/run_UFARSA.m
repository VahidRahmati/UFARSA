function [output_UFARSA, opt_out, ROIs_all] = run_UFARSA(opt, fluor, true_spikes)

% [output_UFARSA, opt_out, ROIs_all] = run_UFARSA(opt, fluor, true_spikes)
%
% Applying UFARSA on the given fluorescence trace(s). The trace(s) will be first pre-processed, and then spiking activity trains will be reconstructed,
% plotted and saved.
%
%
% INPUT:
...
%
%   #Hint#: To save the figures, first a new folder called "UFARSA_results", and then new sub-folder called "Figures" will be created in the folder of data. 
...
%
%   #Hint#: To save these variables, first a new folder called "UFARSA_results", and then a new sub-folder called "Reconstructions" will be created in the folder of data. 
...
%
%   ROIs_all: a cell variable including the UFARSA's outputs (i.e. both output_UFARSA and opt_out) for each of the fluorescence traces; 
%             e.g. to access the outputs for ROI number 3, write ROIs_all{3} in the Matlab Command Window prompt.
%             To remove this output (recommended) use "[output_UFARSA,opt_out] = run_UFARSA(...", instead of "[output_UFARSA,opt_out, ROIs_all] = run_UFARSA(..." 
%   
%   #Hint#: Note that allowing "ROIs_all" to be an output of UFARSA may significantly reduce the speed of the computer, especially in the case of long-term data. This is because 
...
%   #Hint#: If there are more than one fluorescence traces need to be processed, then the output variables "output_UFARSA" and "opt_out" will correspond to the last trace.
%
...
%% read the rest of UFARSA's parameter values
internal_parameters % If desired, user can open the "internal_parameters.m" file, and adjust the values of the rest of UFARSA's parameters
...
for iROI = ROIs_vec
    ...
    if ROIs_all_flag || opt.save_ROIs_all
        ROIs_all{iROI}.output_UFARSA = output_UFARSA;
    ...
    if opt.plt
        ...
        switch sim_flag
            case 1
                plot_UFARSA(output_UFARSA,opt,true_spikes);
            case 0
                h_fig = plot_UFARSA(output_UFARSA,opt);
    ...
    if opt.save_outputs && ~sim_flag
        save_recVectors(output_UFARSA,opt,iROI)
        opt.warn_flag = 0;
    end
    ...
    if opt.display_NoiseSTD; display(sprintf('estimated normalized std_noise for ROI(%d) =  %d', iROI, output_UFARSA.std_noise)), end
    
end

% save the UFARSA's outputs for each fluorescence trace
if opt.save_ROIs_all && sim_flag==0

    if exist([opt.FluorFile_dir,'UFARSA_results'])==0
        mkdir([opt.FluorFile_dir,'UFARSA_results'])
    end
    
    if exist([opt.FluorFile_dir,'UFARSA_results\Reconstructions\ROIs_all','.mat']) ~= 0
        delete([opt.FluorFile_dir,'UFARSA_results\Reconstructions\ROIs_all','.mat'])
    end
    save([opt.FluorFile_dir,'UFARSA_results\Reconstructions\ROIs_all','.mat'],'ROIs_all')
end
...
if exist([opt.FluorFile_dir,'UFARSA_results\Figures'])==0
    mkdir([opt.FluorFile_dir,'UFARSA_results\Figures'])
end

saveas(h_fig, [opt.FluorFile_dir,'UFARSA_results\Figures\','ROI(',num2str(iROI),')',opt.save_plt_format])

end

function save_recVectors(output_UFARSA, opt,iROI)
...
if exist([opt.FluorFile_dir,'UFARSA_results\Reconstructions'])==0
    mkdir([opt.FluorFile_dir,'UFARSA_results\Reconstructions'])
end
...
if opt.save_eTrains
    ...
    if exist([opt.FluorFile_dir,'UFARSA_results\Reconstructions\eTrains\'])==0
    mkdir([opt.FluorFile_dir,'UFARSA_results\Reconstructions\eTrains\'])
    end
    
    eTrain_times = [[1:opt.nFrames_original]' output_UFARSA.eTrain'];
    event_times_fire = find(output_UFARSA.eTrain>0)';
    event_times_fire = [event_times_fire];
    
    if exist([opt.FluorFile_dir,'UFARSA_results\Reconstructions\eTrains\eTrain_ROI(',num2str(iROI),')',opt.save_output_format ]) ~= 0
        delete([opt.FluorFile_dir,'UFARSA_results\Reconstructions\eTrains\eTrain_ROI(',num2str(iROI),')',opt.save_output_format ])
        delete([opt.FluorFile_dir,'UFARSA_results\Reconstructions\eTrains\eTrain_ROI(',num2str(iROI),')_fire',opt.save_output_format ])
    end
    
    switch opt.save_output_format
        case '.txt'
            dlmwrite([opt.FluorFile_dir,'UFARSA_results\Reconstructions\eTrains\eTrain_ROI(',num2str(iROI),')',opt.save_output_format ],eTrain_times,'delimiter','\t','newline','pc')
            dlmwrite([opt.FluorFile_dir,'UFARSA_results\Reconstructions\eTrains\eTrain_ROI(',num2str(iROI),')_fire',opt.save_output_format ],event_times_fire,'delimiter','\t','newline','pc')
        case '.mat'
            save([opt.FluorFile_dir,'UFARSA_results\Reconstructions\eTrains\eTrain_ROI(',num2str(iROI),')',opt.save_output_format ],'eTrain_times')
            save([opt.FluorFile_dir,'UFARSA_results\Reconstructions\eTrains\eTrain_ROI(',num2str(iROI),')_fire',opt.save_output_format ],'event_times_fire')        
    end
...
if opt.save_eTrains_dem && opt.demerging
    ...
    if exist([opt.FluorFile_dir,'UFARSA_results\Reconstructions\eTrains_dem\'])==0
    mkdir([opt.FluorFile_dir,'UFARSA_results\Reconstructions\eTrains_dem\'])
    end
    
    eTrain_times_dem = [[1:opt.nFrames_original]' output_UFARSA.eTrain_dem'];
    event_times_dem_fire = find(output_UFARSA.eTrain_dem>0)';
    event_times_dem_fire = [event_times_dem_fire];
    
    if exist([opt.FluorFile_dir,'UFARSA_results\Reconstructions\eTrains_dem\eTrain_dem_ROI(',num2str(iROI),')',opt.save_output_format ]) ~= 0
        delete([opt.FluorFile_dir,'UFARSA_results\Reconstructions\eTrains_dem\eTrain_dem_ROI(',num2str(iROI),')',opt.save_output_format ])
        delete([opt.FluorFile_dir,'UFARSA_results\Reconstructions\eTrains_dem\eTrain_dem_ROI(',num2str(iROI),')_fire',opt.save_output_format ])
    end
    
    switch opt.save_output_format
        case '.txt'
            dlmwrite([opt.FluorFile_dir,'UFARSA_results\Reconstructions\eTrains_dem\eTrain_dem_ROI(',num2str(iROI),')',opt.save_output_format ],eTrain_times_dem,'delimiter','\t','newline','pc')
            dlmwrite([opt.FluorFile_dir,'UFARSA_results\Reconstructions\eTrains_dem\eTrain_dem_ROI(',num2str(iROI),')_fire',opt.save_output_format ],event_times_dem_fire,'delimiter','\t','newline','pc')
        case '.mat'            
            save([opt.FluorFile_dir,'UFARSA_results\Reconstructions\eTrains_dem\eTrain_dem_ROI(',num2str(iROI),')',opt.save_output_format ],'eTrain_times_dem')
            save([opt.FluorFile_dir,'UFARSA_results\Reconstructions\eTrains_dem\eTrain_dem_ROI(',num2str(iROI),')_fire',opt.save_output_format ],'event_times_dem_fire')          
    end
end
...
if opt.save_cTrains
    ...
    if exist([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cTrains\'])==0
    mkdir([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cTrains\'])
    end
    
    cTrain_times = [[1:opt.nFrames_original]' output_UFARSA.cTrain'];
    cTrain_times_fire = find(output_UFARSA.cTrain>0);
    cTrain_times_fire = [cTrain_times_fire' output_UFARSA.cTrain(cTrain_times_fire)'];
     
    if exist([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cTrains\cTrain_ROI(',num2str(iROI),')',opt.save_output_format ]) ~= 0
        delete([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cTrains\cTrain_ROI(',num2str(iROI),')',opt.save_output_format ])
        delete([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cTrains\cTrain_ROI(',num2str(iROI),')_fire',opt.save_output_format ])
    end
    
    switch opt.save_output_format
        case '.txt'            
            dlmwrite([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cTrains\cTrain_ROI(',num2str(iROI),')',opt.save_output_format ],cTrain_times,'delimiter','\t','newline','pc')
            dlmwrite([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cTrains\cTrain_ROI(',num2str(iROI),')_fire',opt.save_output_format ],cTrain_times_fire,'delimiter','\t','newline','pc')            
        case '.mat'
            save([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cTrains\cTrain_ROI(',num2str(iROI),')',opt.save_output_format ],'cTrain_times')
            save([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cTrains\cTrain_ROI(',num2str(iROI),')_fire',opt.save_output_format ],'cTrain_times_fire')
    end
end
...
if opt.save_cTrains_dem && opt.demerging
    ...
    if exist([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cTrains_dem\'])==0
    mkdir([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cTrains_dem\'])
    end
    
    cTrain_times_dem = [[1:opt.nFrames_original]' output_UFARSA.cTrain_dem'];
    cTrain_times_dem_fire = find(output_UFARSA.cTrain_dem>0);
    cTrain_times_dem_fire = [cTrain_times_dem_fire' output_UFARSA.cTrain_dem(cTrain_times_dem_fire)'];
    
    if exist([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cTrains_dem\cTrain_dem_ROI(',num2str(iROI),')',opt.save_output_format ]) ~= 0
        delete([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cTrains_dem\cTrain_dem_ROI(',num2str(iROI),')',opt.save_output_format ])
        delete([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cTrains_dem\cTrain_dem_ROI(',num2str(iROI),')_fire',opt.save_output_format ])
    end
    
    switch opt.save_output_format
        case '.txt'            
            dlmwrite([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cTrains_dem\cTrain_dem_ROI(',num2str(iROI),')',opt.save_output_format ],cTrain_times_dem,'delimiter','\t','newline','pc')
            dlmwrite([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cTrains_dem\cTrain_dem_ROI(',num2str(iROI),')_fire',opt.save_output_format ],cTrain_times_dem_fire,'delimiter','\t','newline','pc')            
        case '.mat'
            save([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cTrains_dem\cTrain_dem_ROI(',num2str(iROI),')',opt.save_output_format ],'cTrain_times_dem')
            save([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cTrains_dem\cTrain_dem_ROI(',num2str(iROI),')_fire',opt.save_output_format ],'cTrain_times_dem_fire')
    end
end
...
if opt.save_cFR && opt.gen_FR_count
    ...
    if exist([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cFR\'])==0
    mkdir([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cFR\'])
    end
    
    if exist([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cFR\cFR_ROI(',num2str(iROI),')',opt.save_output_format ]) ~= 0
        delete([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cFR\cFR_ROI(',num2str(iROI),')',opt.save_output_format ])
    end
    
    cFR_vector = [[1:opt.nFrames_original]' output_UFARSA.cFR'];
    
    switch opt.save_output_format
        case '.txt'
            dlmwrite([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cFR\cFR_ROI(',num2str(iROI),')',opt.save_output_format],cFR_vector,'delimiter','\t','newline','pc')
        case '.mat'
            save([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cFR\cFR_ROI(',num2str(iROI),')',opt.save_output_format],'cFR_vector')           
    end
end
...
if opt.save_cFR_dem && opt.gen_FR_count_dem && opt.demerging
    ...
    if exist([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cFR_dem\'])==0
    mkdir([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cFR_dem\'])
    end
    
    if exist([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cFR_dem\cFR_dem_ROI(',num2str(iROI),')',opt.save_output_format ]) ~= 0
        delete([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cFR_dem\cFR_dem_ROI(',num2str(iROI),')',opt.save_output_format ])
    end

    cFR_vector_dem = [[1:opt.nFrames_original]' output_UFARSA.cFR_dem'];
    
    switch opt.save_output_format
        case '.txt'
            dlmwrite([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cFR_dem\cFR_dem_ROI(',num2str(iROI),')',opt.save_output_format],cFR_vector_dem,'delimiter','\t','newline','pc')
        case '.mat'
            save([opt.FluorFile_dir,'UFARSA_results\Reconstructions\cFR_dem\cFR_dem_ROI(',num2str(iROI),')',opt.save_output_format],'cFR_vector_dem')
    end
end
...
if opt.save_Ft_orig_norm
    ...
    if exist([opt.FluorFile_dir,'UFARSA_results\Reconstructions\Ft_normalized\'])==0
    mkdir([opt.FluorFile_dir,'UFARSA_results\Reconstructions\Ft_normalized\'])
    end
    
    if exist([opt.FluorFile_dir,'UFARSA_results\Reconstructions\Ft_normalized\Ft_normalized_ROI(',num2str(iROI),')',opt.save_output_format ]) ~= 0
        delete([opt.FluorFile_dir,'UFARSA_results\Reconstructions\Ft_normalized\Ft_normalized_ROI(',num2str(iROI),')',opt.save_output_format ])
    end
    
    Ft_orig_norm = [[1:opt.nFrames_original]' output_UFARSA.fluors.normalized'];
    
    switch opt.save_output_format
        case '.txt'
            dlmwrite([opt.FluorFile_dir,'UFARSA_results\Reconstructions\Ft_normalized\Ft_normalized_ROI(',num2str(iROI),')',opt.save_output_format],Ft_orig_norm,'delimiter','\t','newline','pc')
        case '.mat'
            save([opt.FluorFile_dir,'UFARSA_results\Reconstructions\Ft_normalized\Ft_normalized_ROI(',num2str(iROI),')',opt.save_output_format],'Ft_orig_norm')
    end
end
...
if opt.save_Ft_beforeSmth
    ...
    if exist([opt.FluorFile_dir,'UFARSA_results\Reconstructions\Ft_beforeSmoothing\'])==0
    mkdir([opt.FluorFile_dir,'UFARSA_results\Reconstructions\Ft_beforeSmoothing\'])
    end
    
    if exist([opt.FluorFile_dir,'UFARSA_results\Reconstructions\Ft_beforeSmoothing\Ft_beforeSmoothing_ROI(',num2str(iROI),')',opt.save_output_format ]) ~= 0
        delete([opt.FluorFile_dir,'UFARSA_results\Reconstructions\Ft_beforeSmoothing\Ft_beforeSmoothing_ROI(',num2str(iROI),')',opt.save_output_format ])
    end
    
    Ft_pre_smth = [[1:opt.nFrames_original]' output_UFARSA.fluors.beforeSmoothing'];
    
    switch opt.save_output_format
        case '.txt'
            dlmwrite([opt.FluorFile_dir,'UFARSA_results\Reconstructions\Ft_beforeSmoothing\Ft_beforeSmoothing_ROI(',num2str(iROI),')',opt.save_output_format],Ft_pre_smth,'delimiter','\t','newline','pc')
        case '.mat'
            save([opt.FluorFile_dir,'UFARSA_results\Reconstructions\Ft_beforeSmoothing\Ft_beforeSmoothing_ROI(',num2str(iROI),')',opt.save_output_format],'Ft_pre_smth')
    end
end
...
if opt.save_Ft_afterSmth 
    ...
    if exist([opt.FluorFile_dir,'UFARSA_results\Reconstructions\Ft_afterSmoothing\'])==0
    mkdir([opt.FluorFile_dir,'UFARSA_results\Reconstructions\Ft_afterSmoothing\'])
    end
    
    if exist([opt.FluorFile_dir,'UFARSA_results\Reconstructions\Ft_afterSmoothing\Ft_afterSmoothing_ROI(',num2str(iROI),')',opt.save_output_format ]) ~= 0
        delete([opt.FluorFile_dir,'UFARSA_results\Reconstructions\Ft_afterSmoothing\Ft_afterSmoothing_ROI(',num2str(iROI),')',opt.save_output_format ])
    end
    
    Ft_after_smth= [[1:opt.nFrames_original]' output_UFARSA.fluors.afterSmoothing'];
    
    switch opt.save_output_format
        case '.txt'
            dlmwrite([opt.FluorFile_dir,'UFARSA_results\Reconstructions\Ft_afterSmoothing\Ft_afterSmoothing_ROI(',num2str(iROI),')',opt.save_output_format],Ft_after_smth,'delimiter','\t','newline','pc')
        case '.mat'
            save([opt.FluorFile_dir,'UFARSA_results\Reconstructions\Ft_afterSmoothing\Ft_afterSmoothing_ROI(',num2str(iROI),')',opt.save_output_format],'Ft_after_smth')          
    end
end
