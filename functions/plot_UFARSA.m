function h_fig = plot_UFARSA(output_UFARSA,opt, true_spikes)

% h_fig = plot_UFARSA(output_UFARSA,opt, true_spikes)
%
% This function plots the UFARSA's reconstruction results as well as different versions  
...
%
% INPUT: 
%   output_UFARSA: an structure variable containing UFARSA's reuslts (see the description of run_UFARSA.m function)
%
%   opt: an structure variable containing the plotting options (for a complete list see the description of run_UFARSA.m function)
...
if opt.plt_Ft_orig_norm || plt_notAssigned % plot the original, normalized fluorescence trace
  plt_idx = plt_idx + 1;
  h(plt_idx) = plot(1:opt.nFrames_original, output_UFARSA.fluors.normalized,'color', col{8}/255);
  max_fluor_orig_norm = max(output_UFARSA.fluors.normalized);
  min_fluor_orig_norm = min(output_UFARSA.fluors.normalized);
  orig_f = 1;
...
if opt.plt_Ft_beforeSmth % plot the pre-processed fluorescence trace used for smoothing step
  plt_idx = plt_idx + 1;
  h(plt_idx) =plot(1:opt.nFrames_original, output_UFARSA.fluors.beforeSmoothing,'color', col{10}/255);
  max_fluor_beforeSmoothing = max(output_UFARSA.fluors.beforeSmoothing);
  min_fluor_beforeSmoothing = min(output_UFARSA.fluors.beforeSmoothing);
  pre_f = 1;
...
if opt.plt_Ft_afterSmth % plot the smoothed fluorescence trace used for reconstruction step
   plt_idx = plt_idx + 1; 
  h(plt_idx) =plot(1:opt.nFrames_original, output_UFARSA.fluors.afterSmoothing,'color', col{3}/255);
...
if opt.plt_eTrainOnFt % plot the user's selected type of reconstructed event train on the fluorescence trace(s)
     if opt.plt_eTrain 
        evet_times_rec = find(output_UFARSA.eTrain>0);
     ...
     elseif opt.plt_eTrain_dem
        evet_times_rec = find(output_UFARSA.eTrain_dem>0);
        plot(evet_times_rec, max_fluor_plt*ones(1,numel(evet_times_rec)), 'x', 'LineWidth',2, 'MarkerEdgeColor',col{12}/255,'MarkerSize',10);   
     end
...
if (opt.plt_cTrain || opt.plt_cTrain_dem || opt.plt_cFR || opt.plt_cFR_dem  || opt.plt_cTrain_sim) && opt.ZeroEvent_flag==0
    ...
    if opt.plt_cTrain % plot reconstructed spike-count train without demerging
        count_times_rec = find(output_UFARSA.cTrain>0);
        for ii = 1:numel(count_times_rec)
            if ii==1
                plt_idx = plt_idx + 1;
                h(plt_idx) = line([count_times_rec(ii) count_times_rec(ii)],...
                    [0 output_UFARSA.cTrain(count_times_rec(ii))], 'color', col{5}/255, 'linewidth', 1.5);
            else
                h(plt_idx) = line([count_times_rec(ii) count_times_rec(ii)],...
                    [0 output_UFARSA.cTrain(count_times_rec(ii))], 'color', col{5}/255, 'linewidth', 1.5);
            end
    ...
    if opt.plt_cTrain_dem && opt.demerging % plot the reconstructed spike-count train with demerging
        count_times_rec_SR = find(output_UFARSA.cTrain_dem>0);
        for ii = 1:numel(count_times_rec_SR)
            if ii==1
                plt_idx = plt_idx + 1;
                h(plt_idx) = line([count_times_rec_SR(ii) count_times_rec_SR(ii)],...
                    [0 output_UFARSA.cTrain_dem(count_times_rec_SR(ii))], 'color', col{5}/255, 'linewidth', 1.5);
            else
                h(plt_idx) = line([count_times_rec_SR(ii) count_times_rec_SR(ii)],...
                    [0 output_UFARSA.cTrain_dem(count_times_rec_SR(ii))], 'color', col{5}/255, 'linewidth', 1.5);
            end
    ...
    if opt.plt_cFR && opt.gen_FR_count % plot the estimated firing rate vector, based on reconstructed spike-count without demerging
        plt_idx = plt_idx + 1;
        h(plt_idx)= plot(1:opt.nFrames_original,output_UFARSA.cFR, 'LineWidth',2, 'Color',col{2}/255);
    ...
    if opt.plt_cFR_dem && opt.demerging && opt.gen_FR_count_dem % plot the estimated firing rate vector, based on reconstructed spike-counts with demerging
        plt_idx = plt_idx + 1;
        h(plt_idx)= plot(1:opt.nFrames_original,output_UFARSA.cFR_dem, 'LineWidth',2, 'Color',col{2}/255);
