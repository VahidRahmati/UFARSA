function [fluors, opt] = preprocessing_UAFASAR(opt,fluor)
...
if sum(abs(fluor)) ~= 0
    ...
    if opt.remove_drifts
        switch opt.detrending_method
            case 1 % BEADS method
                detrend_fluor = detrending_beads_UFASAR(fluor', opt)';
    ...
    %% Smoothing
    [fluor_smth, Noise_trace, smoothing_param] = smoothing_UFASAR(opt,fluor_preSmth);
