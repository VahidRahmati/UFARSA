function [detrend_sginal] = detrending_beads_UFASAR(fluor, opt)

% [detrend_sginal] = detrending_beads_UFASAR(fluor, opt)
% 
...
fluor = [fluor(1)*ones(m,1); fluor; fluor(end)*ones(m,1)];
...
switch opt.beads_fast
    case 1 % use our fast implementation
        [x1, f1] = beadsFast_UFASAR(fluor, opt.fc_beads, lam0);
